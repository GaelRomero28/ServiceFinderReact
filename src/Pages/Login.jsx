...
const Login = (props) => {
  ...
  const iniciarSesion = async() => {
    await axios.post(baseURL,inicioSesion)
    .then(response=>{
      ...
      if(response.length>0){
        ...
        if(respuesta.idRol==2){
          axios.get(infoCliente +"?id=" + respuesta.idUsuario)
          .then(response=>{
            return response.data;
          }).then(response=>{
            if(response.length>0){
              ...
              cookies.set("idCliente",respuestaCliente.idCliente,{path:'/'})
              navigate("/Inicio")
            }
          })
        }
        if(respuesta.idRol==3){
          axios.get(infoProfesionista +"?id=" + respuesta.idUsuario)
          .then(response=>{
            return response.data;
          }).then(response=>{
            if(response.length>0){
              ...
              cookies.set("idCliente",respuestaProfesionista.idProfesionista,{path:'/'})
              cookies.set("idProfesion",respuestaProfesionista.idProfesion,{path:'/'})
              cookies.set("descripcion",respuestaProfesionista.descripcion,{path:'/'})
              navigate("/Inicio")
            }
          })
        }
        ...
        Swal.fire({
          text: 'Contraseña o Correo incorrecto, porfavor intente de nuevo',
          icon: 'error',
